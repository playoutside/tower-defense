...
var GameEngine = function GameEngine (_level) {
  ...
  this.sendFullStatus = function sendFullStatus (socket) {
    ...
    fullStatus.credits = that.credits;
    fullStatus.health = that.health;
    fullStatus.wave = that.wave;
    ...
    if (objectType === 'Players') {
      ...
      _level.turretSites.forEach(function turretSitesForEach (site) {
        if (_level.getProximity(entity.pos, site.position) < 25) {
          ...
          action.action = site.tower ? 'Towers.upgrade' : 'Towers.build';
          action.cost = site.tower ? _level.tower.cost + _level.tower.costIncrement * site.tower.level : _level.tower.cost;
          action.currentLevel = 0;

          if (site.tower && site.tower.id) {
            action.towerId = site.tower.id;
            action.currentLevel = site.tower.level;
          }
