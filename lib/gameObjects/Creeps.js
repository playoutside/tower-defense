...
var Creeps = function Creeps () {
  ...
  this.loopActions = {
    creepsLoopAction: function creepsLoopAction (done) {
      if (!that.gameEngine.level) {
        return done();
      }

      var curGameTime = new Date().valueOf() - startTime;
      var intervalTimeStamp = curGameTime % (that.gameEngine.level.wave.delay * 1000 + that.gameEngine.level.wave.duration * 1000);
      /**
       *
       "wave":{"duration":15,"delay":5,"creepCount":20},
       "creep":{"hitPoints":100,"increment":5},
       */
      if (intervalTimeStamp >= that.gameEngine.level.wave.delay * 1000) {
        ...
        if (waveStatus.running) {
          ...
          console.log('now we are between 2 waves', curGameTime);
        }
      }
