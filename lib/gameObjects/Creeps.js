...
var Creeps = function Creeps (gameEngine) {
  ...
  this.hit = function hit (id, damage) {
    var activeCreep = _.findWhere(activeCreeps, {id:id});
    if (!activeCreep) {
      console.log('Trying to hit a non-existing creep: ' + id);
    }
    //console.log('Hitting creep [' + id + '] for [' + damage + '] damage, had [' + activeCreep.hitPoints + '] HP');
    activeCreep.hitPoints -= damage;
    if (activeCreep.hitPoints <= 0) {
      activeCreeps.splice(activeCreeps.indexOf(activeCreep), 1);
      gameEngine.io.emit('Creeps.remove', [{id: id}]);
    }
  };
    ...
    spawn: function spawnCreeps (done) {
      ...
      if (intervalTimeStamp >= gameEngine.level.wave.delay * 1000) {
        ...
        if (! waveStatus.running) {
          ...
          console.log('now we are in a wave', curGameTime);
        }
    ...
    move: function moveCreeps(done) {
      ...
      _.each(activeCreeps, function eachActiveCreeps (activeCreep, key) {
        ...
        if (distanceToMove >= gameEngine.level.pathMetrics.length) {
          // ToDo: reduce players healthPoints
